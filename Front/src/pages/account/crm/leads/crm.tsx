...
export function CRM({ leads }: { leads: Lead[] }) {
  ...
  // WhatsApp mass send feedback state
  const [whatsappMassStatus, setWhatsappMassStatus] = useState<"idle" | "loading" | "success" | "error">("idle");
  const [whatsappMassCount, setWhatsappMassCount] = useState(0);
  ...
  // ── Load email history from backend ──────────────────────────────────────
  useEffect(() => {
    let cancelled = false;
  ...
  // ── Send email (local state) ──────────────────────────────────────────────
  const handleSendEmail = useCallback((emailData: Omit<Email, "id">) => {
    const newEmail: Email = {
      id: Date.now(),
      ...emailData,
      status: emailData.status === "scheduled" ? "scheduled" : "sent",
      sentAt: emailData.status === "scheduled" ? null : new Date().toISOString(),
    };
  ...
  // ── Backend API helpers ───────────────────────────────────────────────────
  const sendManualEmailApi = useCallback(async (
    leadId: number, to: string, subject: string, body: string
  ) => {
    try {
  ...
  // ── WhatsApp mass API ─────────────────────────────────────────────────────
  const simulateMassWhatsAppApi = useCallback(async (leadIds: number[]): Promise<boolean> => {
    try {
      const r = await fetch(`${API_BASE}/whatsapp/actions/simulate-mass-messages`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ leadIds }),
      });
      if (!r.ok) {
        ...
        console.error("[WhatsApp] API error", r.status, errBody);
        return false;
      }
      return true;
    } catch (e) {
  ...
  const handleCreateSequence = useCallback((
    leadIds: number[], templateIds: string[], intervalDays = 3
  ) => {
    const newSeq: Sequence = {
      id: Date.now(),
      leadIds,
      steps: templateIds.map((tid, i) => ({
        templateId: tid,
        day: i * intervalDays,
    ...
    leadIds.forEach(lid => {
      ...
      if (lead && templateIds[0]) {
        ...
        if (tmpl) {
          ...
          handleSendEmail({
            leadId: lid, to: lead.email, subject, body,
            type: "sequence", sequenceId: newSeq.id, sequenceStep: 0,
            status: "sent",
          });
        }
      }
  ...
  // ── WhatsApp mass send handler ────────────────────────────────────────────
  // FIX: case-insensitive status check + trim() guard on phone
  const handleMassWhatsApp = useCallback(async () => {
    const hotLeads = leads
      .filter(l => l.status?.toLowerCase() === "hot" && l.phone?.trim())
      .slice(0, 50);

    if (hotLeads.length === 0) {
      console.warn("[WhatsApp] No hot leads with phone numbers found.");
      return;
    }

    setWhatsappMassStatus("loading");
    setWhatsappMassCount(hotLeads.length);

    const ok = await simulateMassWhatsAppApi(hotLeads.map(l => l.id));

    // Optimistic local registration — batched in a single setState call
    ...
    setEmails(prev => [
      ...hotLeads.map(lead => ({
        ...
        leadId: lead.id,
        to: lead.email,
        subject: `[WhatsApp] Intro — ${lead.company}`,
        body: `Bonjour ${lead.name?.split(" ")[0] ?? ""},\n\nJe suis Abderrahim d'ELBAHI.NET. J'aimerais échanger 10 min sur votre stratégie digitale pour ${lead.company}.\n\n🌐 www.elbahi.net`,
    ...
    setWhatsappMassStatus(ok ? "success" : "error");
    setTimeout(() => setWhatsappMassStatus("idle"), 3000);
  }, [leads, simulateMassWhatsAppApi]);
              ...
              {[
                {
                  ...
                  sub: `${leads.filter(l => l.status?.toLowerCase() === "hot" && !emails.some(e => e.leadId === l.id)).length} à contacter`,
                  onClick: () => {
                    const hotLeads = leads
                      .filter(l => l.status?.toLowerCase() === "hot" && !emails.some(e => e.leadId === l.id))
                      .slice(0, 5);
                    simulateMassEmailsApi(hotLeads.map(l => l.id));
                    hotLeads.forEach(lead => {
                      const { subject, body } = applyTemplate(emailTemplates[0], lead);
                      handleSendEmail({ leadId: lead.id, to: lead.email, subject, body, type: "bulk", status: "sent" });
                    });
                  },
                  status: null,
                },
                
                {
                  ...
                  // FIX: case-insensitive + trim guard
                  sub: whatsappMassStatus === "loading"
                    ? "Envoi en cours…"
                    : whatsappMassStatus === "success"
                    ? `✓ ${whatsappMassCount} envoyés`
                    : whatsappMassStatus === "error"
                    ? "⚠ Erreur API"
                    : `${leads.filter(l => l.status?.toLowerCase() === "hot" && l.phone?.trim()).length} prospects chauds`,
                  onClick: handleMassWhatsApp,
                  status: whatsappMassStatus,
                },
              ].map((action, i) => {
                const isWA = i === 2;
                const statusColor =
                  action.status === "success" ? "#25d366"
                  : action.status === "error"   ? "#f87171"
                  : action.status === "loading" ? "#fbbf24"
                  : undefined;
                return (
                  <button
                    key={i}
                    onClick={action.onClick}
                    disabled={isWA && action.status === "loading"}
                    className="flex flex-col items-center gap-1.5 rounded-xl border border-slate-200 bg-white px-3 py-3 text-center text-xs font-semibold text-slate-700 transition hover:border-slate-400 hover:bg-slate-50 disabled:opacity-60 disabled:cursor-not-allowed"
                    style={statusColor ? { borderColor: `${statusColor}60`, background: `${statusColor}08` } : {}}
                  >
                    <span
                      className="flex size-8 items-center justify-center rounded-lg bg-slate-100 text-slate-500"
                      style={statusColor ? { background: `${statusColor}18`, color: statusColor } : {}}
                    >
                    ...
                    <span
                      ...
                      style={statusColor ? { color: statusColor, fontWeight: 600 } : {}}
                    >
                      {action.sub}
                    </span>
                  </button>
                  ...
                  <div key={seq.id} className="rounded-lg border border-slate-200 bg-white p-3">
                    <div className="mb-2 flex items-center justify-between">
                      <span className="text-xs font-semibold text-slate-700">
                        {seq.leadIds.length} prospects • {seq.steps.length} étapes
                      </span>
                      ...
                      <div key={email.id} className="space-y-1 px-4 py-3 hover:bg-slate-50">
                        <div className="flex items-start justify-between gap-3">
                          <div className="min-w-0 flex-1">
                            <div className="flex items-center gap-2 text-sm font-semibold text-slate-800">
                              {isWhatsApp && (
                                <i className="fa-brands fa-whatsapp text-[12px] text-slate-500" />
                              )}
                        ...
                        {email.type !== "manual" && (
                          <span className="mt-1 inline-block rounded bg-slate-100 px-2 py-0.5 text-[9px] font-semibold uppercase text-slate-500">
                            {email.type === "sequence" ? "Séquence"
                              : email.type === "bulk"     ? "Envoi groupé"
                              : email.type === "followup" ? "Relance"
                              : email.type === "whatsapp" ? "WhatsApp"
                              : email.type}
                          </span>
                        )}
                      </div>
          ...
          <Card>
            ...
            <CardContent className="space-y-2">
              {[
                {
                  l: "Non contactés",
                  fa: "fa-regular fa-circle",
                  count: leads.filter(l => !emails.some(e => e.leadId === l.id)).length,
                },
                {
                  l: "Email envoyé",
                  fa: "fa-solid fa-paper-plane",
                  count: leads.filter(l => emails.some(e => e.leadId === l.id && e.status === "sent")).length,
                },
                {
                  l: "En séquence",
                  fa: "fa-solid fa-rotate-right",
                  count: leads.filter(l => sequences.some(s => s.status === "active" && s.leadIds.includes(l.id))).length,
                },
                {
                  l: "En attente réponse",
                  fa: "fa-solid fa-hourglass-half",
                  count: emails.filter(e => e.status === "sent" && !e.repliedAt).length,
                },
              ].map(s => (
                <div
                  key={s.l}
        ...
        >
          <div
            ...
            style={{
              ...
              background: "#0A3A40",
              border: "1px solid rgba(102,195,158,0.12)",
          ...
          >
            ...
            <div style={{
              padding: "18px 24px",
              borderBottom: "1px solid rgba(102,195,158,0.1)",
              ...
              <button
                onClick={() => setEmailCompose(null)}
                style={{
                  background: "rgba(102,195,158,0.06)", border: "none", borderRadius: 8,
                  width: 32, height: 32, display: "flex", alignItems: "center",
                  justifyContent: "center", cursor: "pointer", color: "rgba(227,255,204,0.45)",
                }}
              >
            ...
            <div style={{ padding: "20px 24px", flex: 1, overflowY: "auto", display: "flex", flexDirection: "column", gap: 14 }}>
              ...
              {/* Recipient */}
              <div>
                <label style={{ fontSize: 11, fontWeight: 700, color: "rgba(227,255,204,0.35)", textTransform: "uppercase", letterSpacing: "0.06em", marginBottom: 6, display: "block" }}>
                  Destinataire
                </label>
                <select
                    ...
                    try {
                      const res = await fetch("https://api.anthropic.com/v1/messages", {
                        ...
                        body: JSON.stringify({
                          ...
                          messages: [{
                            role: "user",
                            content: `Génère un objet et corps d'email de prospection pour ELBAHI.NET. Prospect: ${lead.name}, ${lead.role}, ${lead.company} (${lead.sector}, ${lead.city}). Réponds en JSON: {"subject":"...","body":"..."}. Max 120 mots pour le body. Signe: Abderrahim, ELBAHI.NET`,
                          }],
                        }),
                      });
                      ...
                      const fallbackLead = leads.find(l => l.id === emailCompose.leadId)!;
                      const { subject, body } = applyTemplate(emailTemplates[0], fallbackLead);
                      setEmailCompose(prev => prev ? { ...prev, subject, body } : prev);
                    }
                  }}
                  style={{
                    padding: "10px 16px", background: "rgba(158,219,185,0.08)",
                    border: "1px solid rgba(158,219,185,0.15)", borderRadius: 8,
                    color: "#9EDBB9", fontSize: 12, fontWeight: 700, cursor: "pointer",
                    display: "flex", alignItems: "center", gap: 6,
                    fontFamily: "'Urbanist', sans-serif", alignSelf: "flex-start",
                  }}
                >
              ...
              {/* Attachments */}
              <div>
                <label style={{ fontSize: 11, fontWeight: 700, color: "rgba(227,255,204,0.35)", textTransform: "uppercase", letterSpacing: "0.06em", marginBottom: 8, display: "block" }}>
                  Pièces jointes
                </label>
                <div
                  ...
                  onDrop={e => {
                    ...
                    setEmailCompose(prev => prev
                      ? { ...prev, attachments: [...(prev.attachments || []), ...files.map(f => ({ name: f.name, size: f.size, type: f.type, file: f }))] }
                      : prev);
                  }}
                ...
                >
                  <input
                    id="crm-file-upload" type="file" multiple hidden
                    onChange={e => {
                  ...
                  <div style={{ fontSize: 12, fontWeight: 600, color: "rgba(227,255,204,0.45)" }}>
                    Glisser-déposer ou <span style={{ color: "#8DD4B0" }}>parcourir</span>
                  </div>
                ...
                {(emailCompose.attachments || []).length > 0 && (
                  <div style={{ marginTop: 10, display: "flex", flexDirection: "column", gap: 6 }}>
                    {emailCompose.attachments!.map((att, idx) => {
                      const sizeStr = att.size > 1_000_000
                        ? `${(att.size / 1_000_000).toFixed(1)} MB`
                        : `${Math.round(att.size / 1000)} KB`;
                      return (
                        <div key={idx} style={{ display: "flex", alignItems: "center", gap: 10, padding: "8px 12px", background: "rgba(102,195,158,0.04)", border: "1px solid rgba(102,195,158,0.1)", borderRadius: 8 }}>
                          ...
                          <button
                            onClick={() => setEmailCompose(prev => prev
                              ? { ...prev, attachments: prev.attachments!.filter((_, i) => i !== idx) }
                              : prev)}
                            style={{ background: "none", border: "none", cursor: "pointer", color: "rgba(227,255,204,0.3)", padding: 4 }}
                          >
                            <i className="fa-solid fa-xmark" />
                          </button>
                        </div>
                ...
                <div style={{ display: "flex", gap: 8 }}>
                  {[
                    ...
                    return (
                      <button
                        key={ch.k}
                        onClick={() => setEmailCompose(prev => prev ? { ...prev, channel: ch.k } : prev)}
                        style={{
                          flex: 1, padding: "10px", borderRadius: 10, cursor: "pointer",
                          background: active ? `${ch.color}12` : "rgba(227,255,204,0.03)",
                          border: active ? `1.5px solid ${ch.color}40` : "1.5px solid rgba(102,195,158,0.08)",
              ...
              {(emailCompose.channel === "whatsapp" || emailCompose.channel === "both") && (
                <div style={{ padding: "14px 16px", background: "rgba(37,211,102,0.04)", border: "1px solid rgba(37,211,102,0.12)", borderRadius: 10 }}>
                  ...
                  />
                  <div style={{ fontSize: 10, color: "rgba(227,255,204,0.2)", marginTop: 4 }}>
            ...
            <div style={{ padding: "14px 24px", borderTop: "1px solid rgba(102,195,158,0.1)", display: "flex", justifyContent: "space-between", alignItems: "center", background: "rgba(102,195,158,0.03)" }}>
              <div style={{ fontSize: 11, color: "rgba(227,255,204,0.25)" }}>
                {emailCompose.leadId
                  ? `→ ${leads.find(l => l.id === emailCompose.leadId)?.email || ""}`
                  : "Sélectionnez un destinataire"}
                {(emailCompose.attachments?.length ?? 0) > 0 && (
                  <span style={{ marginLeft: 8, fontSize: 10, fontWeight: 600, color: "#8DD4B0" }}>
                    📎 {emailCompose.attachments!.length}
                  </span>
                )}
              </div>
              <div style={{ display: "flex", gap: 8 }}>
                <button
                  onClick={() => setEmailCompose(null)}
                  style={{ padding: "9px 18px", background: "rgba(102,195,158,0.06)", border: "1px solid rgba(102,195,158,0.1)", borderRadius: 8, color: "rgba(227,255,204,0.55)", fontSize: 13, fontWeight: 600, cursor: "pointer", fontFamily: "'Urbanist', sans-serif" }}
                >
                  Annuler
                </button>
                ...
                {(emailCompose.channel === "whatsapp" || emailCompose.channel === "both") && (
                  <button
                    onClick={async () => {
                      ...
                      const message = emailCompose.whatsappBody  || emailCompose.body;
                      await simulateMassWhatsAppApi([emailCompose.leadId]);
                      handleSendEmail({
                        leadId: emailCompose.leadId,
                        to: emailCompose.to,
                        type: "whatsapp",
                        subject: `[WhatsApp] ${emailCompose.subject || "Message"}`,
                        body: message,
                        status: "sent",
                ...
                {emailCompose.channel !== "whatsapp" && (
                  <button
                    ...
                    style={{
                      padding: "9px 22px",
                      background: (emailCompose.leadId && emailCompose.subject && emailCompose.body)
                        ? "linear-gradient(135deg, #66C39E, #0A3A40)"
                        : "rgba(227,255,204,0.04)",
                      border: "none", borderRadius: 8,
                      color: emailCompose.leadId ? "#fff" : "rgba(227,255,204,0.2)",
                      fontSize: 13, fontWeight: 700,
                      cursor: (emailCompose.leadId && emailCompose.subject && emailCompose.body) ? "pointer" : "not-allowed",
                      display: "flex", alignItems: "center", gap: 6,
                      fontFamily: "'Urbanist', sans-serif",
                      boxShadow: (emailCompose.leadId && emailCompose.subject && emailCompose.body)
                        ? "0 4px 16px rgba(102,195,158,0.25)"
                        : "none",
                    }}
                  >
